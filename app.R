...
accepted_filetypes <- c("png", "jpg", "jpeg", "PNG", "JPG", "JPEG", "Png", "Jpg", "Jpeg", "heic", "HEIC", "Heic")
...
ui <- fluidPage(
  ...
  titlePanel("Upload photo for British Cockapoo Society calendar"),
  h4("The photo can be up to 5Mb in size, please do not reduce the size of your photo."),
        ...
        tabPanelBody(
          ...
          fluidRow(
            column(
              ...
              actionButton(
                inputId = "Go", 
                HTML("After filling in the required information <br/> click here to submit")
              )
            )
          )
        ),
    ...
    if(isTruthy(input$file_upload)){
      ...
      if(tools::file_ext(input$file_upload$datapath) %in% accepted_filetypes){
        ...
        shinyFeedback::feedbackDanger(
          ...
          text = paste0(
            "File type must be one of png, jpeg or heic, file type specified was ",
  ...
  observeEvent(input$Go, {
    ...
    if(iv$is_valid()){
      ...
      future_promise({
        ...
        "Thank you, please look out for an email from cockapoocalendar@gmail.com over the next few days to confirm we’ve received your photo [remember to check your junk mail folder]"
